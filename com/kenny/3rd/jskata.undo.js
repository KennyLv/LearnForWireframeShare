 ...
 * 
 * @description makes undo & redo management easy
 * 			For more detail, please see https://github.com/dsimard/jskata#readme
 */
...
(function() {
	jsk = {
		...
		// Undo
		undo : function undo() {
			var fct = this.dids && this.dids.length > 0 ? this.dids.pop()
					: null;
		...
		// Redo
		redo : function redo() {
			var fct = this.undids && this.undids.length > 0 ? this.undids.pop()
					: null;
			if (this.isFct(fct.redo)) {
		...
		fireEvents : function() {
			...
			if (this.dids.length === 0 && this.undids.length === 0) {
				this.onEmpty();
			}
		},
		// is Function
