...
(function () {
    ...
    Raphael.fn.g.text = function (x, y, text, customattr) {
        return this.text(x, y, text).attr(this.g.txtattr).attr(customattr ? customattr : {});
    };
     ...
     * Do draw the legend.
     * 
     * @param x       the x of the left-top item
     * @param y       the y of the left-top item
     * @param w       the width of legend area
     * @param labels  the array of labels which would be used for legend
     * @param colors  the array of colors
     * @param mark    the mark which symbol is used to show
     */
    Raphael.fn.g.drawlegend = function (x, y, w, labels, colors, mark) {
        var paper = this, 
            glabels = paper.set(),
            poslbs = [];
        ...
        var mw = -1, box = null;
        var rl = bx, rt = y, rw = 0, rh = 0;
        for (var i = 0; i < labels.length; i++) {
            var clr = colors[i];
            glabels.push(paper.set());
            glabels[i].push(paper.g[mark](bx + 5, h, 5).attr({fill: clr, stroke: "none"}));
            glabels[i].push(paper.text(bx + 20, h, labels[i]).attr(paper.g.txtattr).attr({fill: clr || "#000", "text-anchor": "start"}));
            ...
            box = glabels[i][1].getBBox();
            mw = mw < box.width ? box.width : mw;
            bx += box.width + 40;
        }
        rw = bx; rh = box ? box.height * 1.2: rh;
        if(w){
            var o = 20, n = Math.floor(w / (mw + o));
            for(var i = 0, j = 0; i < labels.length; i++) {
                if(j == n){
                    box = glabels[i][1].getBBox();
                    h += box.height * 1.2;
                    j = 0;
                ...
                glabels[i].items[1].attr({x: bx + 20 + j * (mw + o), y: h});
                glabels[i].items[0].attr({x: bx + 5 + j * (mw + o), y: h});
                
                poslbs[i][0].x = bx + 5 + j * (mw + o);
                poslbs[i][0].y = h;
                poslbs[i][1].x = bx + 20 + j * (mw + o);
                poslbs[i][1].y = h;
                j++;
            }
        }
        glabels.rect = paper.rect(rl, rt, rw, rh);
        glabels.item = paper.image("images/cleardot.gif", rl, rt, rw, rh); //{fill: "#EEE","stroke-opacity": 0, "stroke-width":0} //images/cleardot.gif",
        glabels.rect.attr({"stroke-width":1, "stroke":"red"});
        glabels.poslbs = poslbs;
        
        glabels.position = function(x, y){
            for(var i = 0; i < labels.length; i++) {
                var pos0 = poslbs[i][0], pos1 = poslbs[i][1];
                if(i==0){
                    //console.info(glabels[i].items[0].getBBox().x + "    " + pos0.x);
                }
                glabels[i].items[1].attr({x: pos1.x + x, y: pos1.y + y});
                glabels[i].items[0].attr({x: pos0.x + x, y: pos0.y + y});
            }
            var trl = rl + x, 
                trt = rt + y,
                pos = {
            ...
            glabels.item.attr(pos);
            glabels.rect.attr(pos);
        };
        glabels.updatePos = function(x, y){
            for(var i = 0; i < labels.length; i++) {
                var pos0 = poslbs[i][0], pos1 = poslbs[i][1];
                pos0.x += x;
                pos0.y += y;
    ...
    /**
     * Do draw the axis lines or labels. For example:
     *     r.g.axis(350,440,310,null,null,4,2,["Today",  "Yesterday", "Tomorrow", "Future"], "|", 0);
     *     r.g.axis(330,440,300,0,400,10,1);
     * 
     * @param x,        the begin position of x-coordinate
     * @param y,        the begin position of y-coordinate
     * @param length,   the length of crossing area on coordinate
     * @param from,     the begin value of business data
     * @param to,       the end value of business data
     * @param steps,    the total steps between 'from' and 'to'
     * @param orientation, the direction for the axis
     *                             0, is the bottom of the border line
     *                             1, is the left   of the border line 
     *                             2, is the header of the border line 
     *                             3, is the right  of the border line
     * @param labels,   the labels to be shown on the page
     * @param type,     unkown meaning for now
     * @param dashsize, unkown meaning for now
     */
    Raphael.fn.g.axis = function (x, y, length, from, to, steps, orientation, labels, type, dashsize, pathAttr) {
