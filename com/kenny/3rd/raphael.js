...
(function () {
    ...
    if (R.svg) {
        ...
        Element[proto].getBBox = function () {
            ...
            var bbox = {}, cloneNode = null;
            try {
                var svgObj = null;
                var raphaelDivObj = document.getElementById("raphael_temp_div");
                if(!raphaelDivObj){
                    raphaelDivObj = document.createElement("div");
                    raphaelDivObj.id = "raphael_temp_div";
                    raphaelDivObj.style.position = "absolute";
                    raphaelDivObj.style.left = "-5000px";
                    raphaelDivObj.style.top = "-5000px";
                    document.body.appendChild(raphaelDivObj);
                    svgObj = document.createElementNS("http://www.w3.org/2000/svg","svg");
                    raphaelDivObj.appendChild(svgObj);
                }
                else{
                    svgObj = raphaelDivObj.getElementsByTagName("svg")[0];
                    var childs = svgObj.childNodes;
                    for(var i = childs.length-1; i > -1; i--){
                        svgObj.removeChild(childs[i]);
                    }
                }
                cloneNode = this.node.cloneNode(true);
                svgObj.appendChild(cloneNode);
                bbox = cloneNode.getBBox();
            } 
            catch(e){
               //For firefox 3.x  
                //throw e;
            ...
            if (this.type == "text") {
                ...
                for (var i = 0, ii = cloneNode.getNumberOfChars(); i < ii; i++) {
                    var bb = cloneNode.getExtentOfChar(i);
