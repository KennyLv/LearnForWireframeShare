...
com.kenny.util.BaseTool = {	
	 ...
	 * 
	 * //alert(com.kenny.util.BaseTool.CellIndex_NumToStr(com.kenny.util.BaseTool.CellIndex_StrToNum("ERBQWIOERBL"),
	 * ""));
	 */
    ...
    markSvgTextSelection: function (selection) {
    	...
    	// Same span
    	if(fromObj == toObj){
    		var rect = com.kenny.util.BaseTool.getBoundingRectForSvg(fromObj);
    		currentData = {};
    		...
    		for(var i=0; i<textObj.childNodes.length; i++){
    			...
    			if(obj == fromObj){
    				...
    				var rect = com.kenny.util.BaseTool.getBoundingRectForSvg(obj);
    	...
    	function getLineHeight(objs, y){
    		...
    		for(var i=0; i<objs.length; i++){
        		...
        		if(ny == y){
        			var rect = com.kenny.util.BaseTool.getBoundingRectForSvg(tspan);
    ...
    markTextSelection: function (selection) {
        ...
        function makeSpanSelection(obj) {
            ...
            if (spanPaddingLeft) {
                // try{
                left += com.kenny.util.BaseTool.convertPixelToNumber(spanPaddingLeft);
                // }
                // catch(e){
                // alert("spanPaddingLeft = "+spanPaddingLeft);
                // }

                fullWidth -= com.kenny.util.BaseTool.convertPixelToNumber(spanPaddingLeft);
            }
            var width = fullWidth;
        ...
        function showOverWordDiv(lineDivObj, left, top, width, height) {
            ...
            with (overWordDiv) {
                ...
                var paddingTop = (lineDivObj.style.paddingTop != '' && lineDivObj.style.paddingTop != null) ? com.kenny.util.BaseTool.convertPixelToNumber(lineDivObj.style.paddingTop) : 0;
                if (paddingTop != 0) {
	...
	actsAsAspect : function (object) {
		var aop = com.kenny.util.AopInit.getAop();
		...
		for (var i = 0; i < events.length; i++) {
			(function(event, item) {
				item.getDomInstance().addEventListener(event, function(e) {
					com.kenny.event.MouseEvent[event](e, item);
				}, true);
			})(events[i], item);
		}
	},
