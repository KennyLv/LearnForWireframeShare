 ...
 */
com.kenny.tracker.ImageTracker = function() {
	...
	com.kenny.util.BaseTool.addForbiddenPropagation(this._flipInstance);
};

com.kenny.tracker.ImageTracker.prototype = {
	...
	_initChilds : function() {
		...
		for (var i = 0; i < rectCursor.length; i++) {
			...
			com.kenny.util.BaseTool.addForbiddenPropagation(btn);
		}
	...
	_updateBtnLayout : function() {
		...
		if(this._dragMode){
			...
			var diffs = com.kenny.util.BaseTool.convertCoordinateXY(deltaX, deltaY, -delta);
	...
	_updateCoordLayout : function() {
		...
		if (this._focusBtnNum == null) {
			this._coord.style.top = com.kenny.util.BaseTool.convertPixelToNumber(this._buttons[0].style.top) - 20;
			this._coord.style.left = com.kenny.util.BaseTool.convertPixelToNumber(this._buttons[0].style.left) + 3;
			this._setCoordValue(this._buttonsPos[0].x, this._buttonsPos[0].y);
		} else {
			this._coord.style.top = com.kenny.util.BaseTool.convertPixelToNumber(this._buttons[this._focusBtnNum].style.top) - 20;
			this._coord.style.left = com.kenny.util.BaseTool.convertPixelToNumber(this._buttons[this._focusBtnNum].style.left) + 3;
	...
	_registerMouseEvent : function() {
		...
		for (var i = 0; i < this._buttons.length; i++) {
			this._buttons[i].onmousedown = function(evt) {
				...
				self._sendMessage(com.kenny.util.Observer.MessageType.RULER_DISPLAY, self, {'display':true});
				self.lineShow();
				self.coordShow();
				...
				var mousedownPos = com.kenny.util.BaseTool.getRelativeCoordinates(evt,self.getActiveItem().getDomInstance().parentNode);
				...
				self.getActiveItem().getDomInstance().parentNode.onmousemove = function(evt) {
					evt = evt || window.event;
					var movePos = com.kenny.util.BaseTool.getRelativeCoordinates(evt,self.getActiveItem().getDomInstance().parentNode);
					
					movePos.x -= centerPosition.x;
					movePos.y -= centerPosition.y;
					diffH = y - evt.pageY, diffW = x - evt.pageX;
					var diffs = com.kenny.util.BaseTool.getOffsetWithDelta(movePos.x, movePos.y, mousedownPos.x, mousedownPos.y, delta);
				...
				document.body.onmouseup = function(evt) {
					...
					self._sendMessage(com.kenny.util.Observer.MessageType.RULER_DISPLAY, self, {'display':false});
					self.lineHide();
					self.coordHide();
					...
					//Get the point of 
					var targetCenterPos = com.kenny.util.BaseTool.convertCoordinateXY(trackerCenterRelativeX, trackerCenterRelativeY, delta*(-1));
					...
					self._trackerType ? self._sendMessage(com.kenny.util.Observer.MessageType.IMAGE_RESIZE, self, data) 
							: self._sendMessage(com.kenny.util.Observer.MessageType.IMAGE_CLIP, self, data);
	...
	_registerDraggable : function() {
		...
		var options = {
			...
			drag : function(event, ui) {
				//var pos1 = com.kenny.util.BaseTool.getAbsPostionInContainer(self._activeItem.getWinInstance());
				var pos2 = com.kenny.util.BaseTool.getAbsPostionInContainer(self._activeItem.getDomInstance().parentNode);
	...
	_registerFlipEvent : function() {
		...
		$(this._flipInstance).click(function(e){
			var pos = com.kenny.util.BaseTool.getRelativeCoordinates(e, com.kenny.util.BaseTool.findContainer(self.getDomInstance()));
			...
			var t = com.kenny.util.BaseTool.isInsideTriangle(p, a1, b1, c1);
			var l = com.kenny.util.BaseTool.isInsideTriangle(p, a2, b2, c2);
			var r = com.kenny.util.BaseTool.isInsideTriangle(p, a3, b3, c3);
			var b = com.kenny.util.BaseTool.isInsideTriangle(p, a4, b4, c4);
			...
			var data = {'flip' : flip};
			self._sendMessage(com.kenny.util.Observer.MessageType.IMAGE_SETSTYLE, self.getActiveItem(), data);
	...
	setTop : function(top) {
		this._top = com.kenny.util.BaseTool.convertPixelToNumber(top);
		this._updateLayout();
	},
	...
	setLeft : function(left) {
		this._left = com.kenny.util.BaseTool.convertPixelToNumber(left);
		this._updateLayout();
	},
	...
	setWidth : function(width) {
		this._width = com.kenny.util.BaseTool.convertPixelToNumber(width);
		this._updateLayout();
	},
	...
	setHeight : function(height) {
		this._height = com.kenny.util.BaseTool.convertPixelToNumber(height);
		this._updateLayout();
	},
	...
	setActiveItem : function(item) {
		...
		var pos = com.kenny.util.BaseTool.getAbsPostionInContainer(this._activeItem.getDomInstance());
		...
		this._sendMessage(com.kenny.util.Observer.MessageType.CONTEXT_FOCUS, this, {});
		this.show();
