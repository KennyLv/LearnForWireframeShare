 ...
 * @author lu_feng
 */
com.kenny.model.TextSelectionModel = function(from, to, fromOffset, toOffset) {
...
com.kenny.model.TextSelectionModel.prototype = {
	_from:null,
	_to:null,
	_fromOffset:null,
	_toOffset:null,
	...
	clone: function(){
		var cloneObj = new com.kenny.model.TextSelectionModel();
...
com.kenny.model.TextSelectionModelHelper = {
		
	parseWithLineAndWidth: function(lineDivObj, width){
		var fromOffset = -1, fromObj = null;
		
        var paraDivObj = com.kenny.util.BaseTool.findElementWithType(lineDivObj, 'Paragraph');
        var paragraphObj = com.kenny.util.BaseTool.findObjWithId(paraDivObj.id);
		
		var position = 0;
		//var pos = com.kenny.util.BaseTool.getAbsPostionInContainer(lineDivObj);
		var textSpans = $(lineDivObj).children().filter("span");
		for(var i=0; i< textSpans.length; i++){
			var spanItemObj = textSpans[i];
			position = com.kenny.util.BaseTool.getAbsPostionInContainer(spanItemObj).x;
			var paddingLeft = spanItemObj.style.paddingLeft!=''? com.kenny.util.BaseTool.convertPixelToNumber(spanItemObj.style.paddingLeft): 0;
			...
			if(width <= position + paddingLeft){
				fromObj = spanItemObj;
				fromOffset = com.kenny.util.BaseTool.getParentOffsetWithType(spanItemObj, 0, paraDivObj);
				break;
			}
			//Belong to this span
				...
				com.kenny.util.TextHelper.changeStyle(textStyle.style);
				fromOffset += com.kenny.util.TextHelper.getOffsetWithWidth($(spanItemObj).text(), width - position - paddingLeft);
				
				break;
			}
			else if(i == textSpans.length - 1){
				fromObj = spanItemObj;
				fromOffset = com.kenny.util.BaseTool.getParentOffsetWithType($(spanItemObj).text(), $(spanItemObj).text().length, paraDivObj);
				
				break;
			}
		}
		
		var textSelection = new com.kenny.model.TextSelectionModel();
	...
	parseWindowSelection: function(selection, evt, isShift){
		...
		if(isShift || (selection.isCollapsed && $.browser.opera)){
			...
			var relativePos = com.kenny.util.BaseTool.getRelativeCoordinates(evt, $('.docs-editor')[0]);
			//console.log("relativePos = "+JSON.stringify(relativePos));
			var pageDivObj = null;
			var lineDivObj = null;
			
			pages.children().filter("div").each(function(idx){
				var pos = com.kenny.util.BaseTool.getAbsPostionInContainer(this);
			...
			if(pageDivObj.hasChildNodes()){
				...
				if(paragraphDivObj.getAttribute('objectType') != 'Paragraph'){
					paragraphDivObj = com.kenny.util.BaseTool.findNextParagraph(paragraphDivObj);
				}
				var previousParagraph = paragraphDivObj;
				while(paragraphDivObj){
					var position = com.kenny.util.BaseTool.getAbsPostionInContainer(paragraphDivObj);
				...
				if(paragraphDivObj){
					...
					while(lineDivObj){
						var position = com.kenny.util.BaseTool.getAbsPostionInContainer(lineDivObj);
        ...
        toParaDivObj = fromParaDivObj = com.kenny.util.BaseTool.findElementWithType(fromObj, 'Paragraph');
        toOffset = fromOffset = com.kenny.util.BaseTool.getParentOffsetWithType(fromObj, anchorOffset, toParaDivObj);
        to = from = com.kenny.util.BaseTool.findObjWithId(fromParaDivObj.id);
        
        //Only when selection is not collapsed to find focus values.
        if(!selection.isCollapsed){
        	toObj = focusNode.parentNode;
        	toParaDivObj = com.kenny.util.BaseTool.findElementWithType(toObj, 'Paragraph');
            toOffset = com.kenny.util.BaseTool.getParentOffsetWithType(toObj, focusOffset, toParaDivObj);
            to = com.kenny.util.BaseTool.findObjWithId(toParaDivObj.id);
        }
        
        textSelection = new com.kenny.model.TextSelectionModel();
