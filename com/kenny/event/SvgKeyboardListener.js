 ...
 * @import com.kenny.util.BaseTool,com.kenny.event.KeyboardHandler
 * @author lu_feng
 */
if (!com.kenny.event.SvgKeyboardListener) {
	com.kenny.event.SvgKeyboardListener = function() {
		...
		this.getKeyEditor = function(){
			if(this._keyeditor == null){
				...
				inputObj.focus(function(){
					...
					self.sendCursorEvent(null, com.kenny.util.Observer.MessageType.SVG_CURSOR_PAUSE);
				});
		...
		this.releaseListener = function(){
			if(this._activeItem!=null){
				...
				this._activeItem = null;
				//Hide SVG cursor
				this.sendCursorEvent(null, com.kenny.util.Observer.MessageType.SVG_CURSOR_HIDE);
				//Hide svg selection background
				com.kenny.util.BaseTool.hiddenSvgOverWordDiv();
				//release SVG selection event 
				com.kenny.util.SvgSelection.releaseListener();
			}
		};
		...
		this.registerListener = function(message){
			...
			$(self._activeItem.getKeyBoardDomInstance()).bind('keydown.keyboard', function(evt){
				...
				return;
				//release event 
				self.sendCursorEvent(null, com.kenny.util.Observer.MessageType.SVG_CURSOR_HIDE);
				com.kenny.util.BaseTool.hiddenSvgOverWordDiv();
				com.kenny.util.SvgSelection.releaseListener();
			...
			if(self._activeItem.getType() == 'TextBox'){
				com.kenny.util.SvgSelection.registerListener(self._activeItem.getKeyBoardDomInstance(), function(evt){
					self.sendCursorEvent(null, com.kenny.util.Observer.MessageType.SVG_CURSOR_HIDE);
					var popmenuObj = document.getElementById('textBoxMenuId');
					if(popmenuObj) popmenuObj.style.display = 'none';
				}, function(evt){
					...
					var svgSelection = com.kenny.util.SvgSelection.svgSelection;
					var selection = com.kenny.model.SvgTextSelectionModelHelper.parseWindowSelection(svgSelection, evt);
					...
					if(selection.isCollapsed()){
						...
						self.sendCursorEvent(selection, com.kenny.util.Observer.MessageType.SVG_CURSOR_SHOW);
					}
					else{
						self.resetImeStatusData();
						self.sendCursorEvent(selection, com.kenny.util.Observer.MessageType.SVG_CURSOR_HIDE);
					}
				}, function(evt){
				});
				...
				var selection = null;
				if(evt){
					com.kenny.util.SvgSelection.getSVGSelection('anchor',evt);
					var svgSelection = com.kenny.util.SvgSelection.svgSelection;
					selection = com.kenny.model.SvgTextSelectionModelHelper.parseWindowSelection(svgSelection, evt);
				}
				else{
					selection = new com.kenny.model.SvgTextSelectionModel(self._activeItem, 0, 0, 0, 0);
				}
		...
		this.update = function(message) {
			...
			switch(message.id){
				case com.kenny.util.Observer.MessageType.SVG_KEYBOARD_FOCUS:
					this.releaseListener();
					this.registerListener(message);
					break;
				case com.kenny.util.Observer.MessageType.SVG_KEYBOARD_ACTIVE:
					if(this._keyEditorFocusFlag == false){
						this.resetImeStatusData();	
					}
					break;
				case com.kenny.util.Observer.MessageType.KEYBOARD_BLUR:
					this.releaseListener();
					break;
			...
			if(message.id == com.kenny.util.Observer.MessageType.KEYBOARD_INPUT){
				this.resetEditorData();
			}
			//Since keyboard focus would be frequently invoked, ignore it
			else if(isImeAction == false && message.id != com.kenny.util.Observer.MessageType.SVG_KEYBOARD_ACTIVE){
				this.doImeEndAction();
			}
		};
	};
	com.kenny.event.SvgKeyboardListener.prototype = {
		...
		sendTextStyleEvent: function(selection){
		    ...
		    msg.id = com.kenny.util.Observer.MessageType.SVG_TEXT_STYLE_NOTIFY;
		...
		doDeleteAction: function(flag){
			...
			var msgId = com.kenny.util.Observer.MessageType.SVG_TEXT_DELETE;
		...
		doInputAction: function(val, isImeInputFlag){
			...
			if(this._textInputData.length == 0){
	    		...
	    		this.sendKeyboardAction(this._activeItem, com.kenny.util.Observer.MessageType.SVG_TEXT_INPUT, {
	    		...
	    		this.sendKeyboardAction(this._activeItem, com.kenny.util.Observer.MessageType.SVG_TEXT_INPUT_ITEM, {
	    			val: val,
	    			imeModeFlag: imeModeFlag
				});
	    	}
	...
	(function(){
		var keyboardListener = new com.kenny.event.SvgKeyboardListener();
		com.kenny.util.Observer.register(com.kenny.util.Observer.MessageType.SVG_KEYBOARD_FOCUS, keyboardListener);
		com.kenny.util.Observer.register(com.kenny.util.Observer.MessageType.SVG_KEYBOARD_ACTIVE, keyboardListener);
		com.kenny.util.Observer.register(com.kenny.util.Observer.MessageType.KEYBOARD_BLUR, keyboardListener);
