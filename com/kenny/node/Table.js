 ...
 * @import com.kenny.util.BaseTool,com.kenny.util.NodeCache,com.kenny.node.Cell, com.kenny.model.TableSelectionModel
 * @author xuliyu
 */
com.kenny.node.Table = function () {
	com.kenny.node.Table.superClass.constructor.call(this);
	...
	this._activeSelection = new com.kenny.model.TableSelection(0, 0, 0, 0);
	this._cells = [];
	this._cellTracker = new com.kenny.tracker.CellTracker();
	...
	this._domInstance.addEventListener('contextmenu', function(event) {
		com.kenny.event.MouseEvent.rightclick(event, self);
	}, true);
...
com.kenny.util.BaseTool.extend(com.kenny.node.Table,com.kenny.node.Node);

com.kenny.util.BaseTool.augment(com.kenny.node.Table, {
	...
	addSelection: function (selection,mustPush) {
		//    	if(mustPush == null){
		//            com.kenny.model.TableSelectionModelHelper.addSelection(this._selections, selection);
		//    	} else {
		com.kenny.model.TableSelectionModelHelper.addSelection(this._selections, selection,mustPush);
		//    	}
	},
	removeSelection: function (selectionID) {
		com.kenny.model.TableSelectionModelHelper.removeSelection(this._selections, selectionID);
	},
	addSelectionByDel: function (selection) {
		com.kenny.model.TableSelectionModelHelper.addSelectionByDel(this._selections, selection);
	},
	...
	bindSelectRange: function () {
		...
		$(this._domInstance).bind('mousedown', function (e) {
			...
			var targetObj = com.kenny.util.BaseTool.findEventTarget(e);
	...
	refreshFocus: function () {
		var tableFocusMsg = new com.kenny.util.Observer.Message();
		tableFocusMsg.id = com.kenny.util.Observer.MessageType.TABLE_FOCUS;
		tableFocusMsg.sender = this;
		tableFocusMsg.data = {};
		com.kenny.util.Observer.sendMessage(tableFocusMsg);
	},
	addRow: function (rowNo) {
		...
		if (this._selectColNo != null)
			this._cellTracker.refresh(this.getCellsByColNo(this._selectColNo), com.kenny.tracker.CellTracker.FlagType.Vertical);
		...
		com.kenny.model.TableSelectionModelHelper.extendRow(this._selections, rowNo);
		this.render();
	...
	addRowForUndo: function (cells ,rowIndex ,hdRows ,ftRows) {
		...
		this.refreshFocus();
		//   this._cellTracker.refresh(this.getCellsByRowNo(rowIndex), com.kenny.tracker.CellTracker.FlagType.Horizontal);
		this.refreshCellIndex();
		...
		com.kenny.model.TableSelectionModelHelper.extendRow(this._selections, rowIndex);
		this.render();
	...
	deleteRow: function (rowNo) {
		...
		if (this._selectRowNo != null) {
			...
			this._cellTracker.refresh(this.getCellsByRowNo(this._selectRowNo), com.kenny.tracker.CellTracker.FlagType.Horizontal);
		}
		...
		com.kenny.model.TableSelectionModelHelper.removeRow(this._selections, rowNo);
		this.render();
	...
	rowSelect: function (e) {
		...
		this._cellTracker.setFlag(com.kenny.tracker.CellTracker.FlagType.Horizontal);
		this._cellTracker.refresh(this.getCellsByRowNo(this._selectRowNo), com.kenny.tracker.CellTracker.FlagType.Horizontal);
	},
	...
	addCol: function (colNo) {
		...
		com.kenny.model.TableSelectionModelHelper.extendCol(this._selections, colNo);
		this.render();
	...
	addColForUndo: function (cells, colIndex, headerCols) {
		...
		//update
		//this._cellTracker.refresh(this.getCellsByColNo(colIndex), com.kenny.tracker.CellTracker.FlagType.Vertical);
		this.setWidth(this.getWidth() + offset);
		this.refreshFocus();
		this.refreshCellIndex();

		com.kenny.model.TableSelectionModelHelper.extendCol(this._selections, colIndex);
		this.render();
	...
	deleteCol: function (colNo) {
		...
		com.kenny.model.TableSelectionModelHelper.removeCol(this._selections, colNo);
		this.render();
	...
	colSelect: function (e) {
		...
		this._cellTracker.setFlag(com.kenny.tracker.CellTracker.FlagType.Vertical);
		this._cellTracker.refresh(this.getCellsByColNo(this._selectColNo), com.kenny.tracker.CellTracker.FlagType.Vertical);
	},
	...
	createCell: function (r, c, top, left, width, height,background) {
		var cell = new com.kenny.node.Cell();
		cell.setWidth(width);
		cell.setHeight(height);
		cell.setLeft(left);
		cell.setTop(top);
					...
					for (var i = 0; i < cells.length; i++) {
						...
						switch (type) {
							case 'ADDROW':
								//[3-                            //+1
								if (row >= RowOrColNO) {
									expression = expression.replace(cells[i], com.kenny.util.BaseTool.CellIndex_NumToStr(col, "") + (row + 1).toString());
								}
								break;
							case 'DELETEROW':
								//[3-                            //-1
							...
							case 'ADDCOL':
								//-3]                            //+1
								if (col >= RowOrColNO) {
									expression = expression.replace(cells[i], com.kenny.util.BaseTool.CellIndex_NumToStr((col + 1), "") + row);
								}
								break;
							case 'DELETECOL':
								//-3]                            //-1
	...
	refreshCellValue: function () {
		for (var c = 0; c < this._cells.length; c++) {
			...
			if (cell.getFunctions() != null) {
				try {
					var cellValue = com.kenny.util.AlgorithmsParseTools.doCalculate(cell.getExpression().toUpperCase());
					if (!isNaN(cellValue)) {
	...
	setCellStyle: function () {// temp function
		//com.kenny.model.TableSelectionModelHelper.updateTableCellsStyle(this, this._selections);
		if (this._headerRowsColor == null)
			this._headerRowsColor = "#B6B6B6";
		if (this._headerColsColor == null)
			this._headerColsColor = "#D7D7D7";
	...
	setData: function (data) {
		...
		if (data.selections) {
			for (var i = 0; i < data.selections.length; i++) {
				var selection = new com.kenny.model.TableSelection();
