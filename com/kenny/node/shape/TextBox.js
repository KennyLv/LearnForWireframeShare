 ...
 * @import com.kenny.util.BaseTool, com.kenny.render.SvgTextRender, com.kenny.model.SvgTextSelectionModel
 * @author Feng.Lu
 */
com.kenny.node.shape.TextBox = function() {
	this._id = com.kenny.util.BaseTool.uuid();
	this._domInstance = com.kenny.util.ShapeHelper.createTextBox({
		id : this._id,
		top : this._top,
		left : this._left,
		width : this._width,
...
//	this._domInstance.appendChild(this._cancelEditDiv);
	...
	this._activeSelection = new com.kenny.model.SvgTextSelectionModel();
	
	this._render = new com.kenny.render.SvgTextRender(this);
...
com.kenny.node.shape.TextBox.prototype = {
	...
	beginEdit: function(evt){
		...
		message.id = com.kenny.util.Observer.MessageType.SVG_KEYBOARD_FOCUS;
		message.sender = this;
		message.data = {
			evt: evt
		};
		...
		//Hide tracker
		var hidetracker = new com.kenny.util.Observer.Message();
		hidetracker.id = com.kenny.util.Observer.MessageType.SHAPE_TRACKER_HIDE;
		hidetracker.sender = this;
		com.kenny.util.Observer.sendMessage(hidetracker);
		
		//Context active
		var message = new com.kenny.util.Observer.Message();
		message.id = com.kenny.util.Observer.MessageType.CONTEXT_ACTIVE;
		message.sender = this;
		com.kenny.util.Observer.sendMessage(message);
	},
	endEdit: function(){
		...
		$(this._imgInstanceDot).bind('mouseup', function(evt){
			...
			setTimeout(function(){
				if(!obj.hasClass('noClick')){
					com.kenny.event.MouseEvent.click(e, self);
	...
	_resize : function(flag) {
		this._resizeDot();
		this._domInstance = com.kenny.util.ShapeHelper.updateTextBox(
				this._domInstance, {
	...
	setData : function(data) {
		...
		if(data.styles && data.styles.length>0) {
			...
			com.kenny.util.TextStyleContainerUtils.setAllDataForSvgStyle(data.styles, true);
		}
		...
		if(data.texts){
			...
			for(var i=0; i<data.texts.styles.length; i++){
				styles.push({
					...
					style: com.kenny.util.TextStyleContainerUtils.getById(data.texts.styles[i].style).clone()
				});
			}
			this._texts = {
	...
	getData : function() {
		...
		for(var i=0; i<this._texts.styles.length; i++){
			...
			stylesData.style.setId(com.kenny.util.BaseTool.uuid());
			com.kenny.util.TextStyleContainerUtils.register(stylesData.style);
			textStyles.push({
...
//		}
	...
	clone : function() {
		var cloneObj = new com.kenny.shape.TextBoxNew();
