 ...
 * @author lu_feng
 */
com.hoperun.node.Paragraph = function(){
	com.hoperun.node.Paragraph.superClass.constructor.call(this);
...
com.hoperun.util.BaseTool.extend(com.hoperun.node.Paragraph,com.hoperun.node.Node);

com.hoperun.util.BaseTool.augment(com.hoperun.node.Paragraph,{
	_page: null,
	...
	registerStyle: function(){
		for(var i=0; i<this._textContainer.length; i++){
			com.hoperun.util.TextStyleContainerUtils.register(this._textContainer[i].style);
		}
	},
...
//		this._styleContainer.push(style);
	...
	_getDefaultTextStyle: function(){
		if(!this._defaultTextStyle){
			if(this._textContainer.length > 0){
				...
				this._defaultTextStyle = new com.hoperun.node.Style();
			}
		}
	...
	_createDomInstance: function(){
		...
		com.hoperun.util.BaseTool.setStyleProperty(domInstance.style, 'text-align', this._textAlign);
			...
			if(offset > previousOffset && offset <= currentOffset){
				for(var j=0; j<this._linesData[i].data.length; j++){
					...
					if( offset <= previousOffset + textStyleWidth ){
						...
						com.hoperun.util.TextHelper.changeStyle(textStyleObj.style);
						width = com.hoperun.util.TextHelper.getWidth(this._text.substring(previousOffset, offset));
						break;
					}
	...
	_createParagraphDomData: function(paragraphDivObj){
		...
		for(var i=0; i<this._linesData.length; i++){
			var lineObj = new com.hoperun.model.LineModel(this, indentStyle, i);
	...
	updatePage: function(isCascade){
		...
		if(this._linesDataIsChange || !com.hoperun.model.BasicModelHelper.isEqualsList(this._pageAttackItems, attackItems)){
		...
		if(this._linesDataIsChange){
			...
			this._linesData = com.hoperun.util.TextHelper.getLinesOfParagraph(this, this._pageStart);
...
//		}
	...
	breakParagraph: function(offset, paragraphObj){
		if(paragraphObj){
			...
			var paragraphDivObj = com.hoperun.util.BaseTool.findElementWithType(spanObj, this._type);
	...
	clone:function(){
		var cloneObj = new com.hoperun.node.Paragraph();
	...
	setData:function(data){
		...
		if(data.style && data.style.length>0) {
			...
			for(var i=0; i<styleData.length; i++){
				var styleObj = com.hoperun.util.TextStyleContainerUtils.getById(styleData[i].style).clone();
		...
		if(pageItems.current){
			...
			for(var i=0; i<pageItems.current.length; i++){
				...
				if(itemTop >= paragraphTop || itemTop + h > paragraphTop){
					...
					var basicItem = new com.hoperun.model.BasicModel(itemLeft, itemTop, w, h);
