 ...
 * @import com.hoperun.util.NodeCache, com.hoperun.util.BaseTool
 * @author lu_feng
 */
com.hoperun.util.TextStyleContainerUtils = {
	_items: null,
	...
	setAllData: function(data, notIgnoreSameFlag){
		for(var i=0; i <data.length; i++){
			var style = new com.hoperun.node.Style();
	...
	setAllDataForSvgStyle: function(data, isNeedAllFlag){
		if(isNeedAllFlag){
			var styles = this.getAll();
			for(var i=0; i <data.length; i++){
				var style = new com.hoperun.model.SvgStyle();
				style.setData(data[i]);
				style.setId(data[i].id);
				styles.push(style);	
			}
