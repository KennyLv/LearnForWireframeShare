 ...
 * @author lu_feng
 */
if (!com.hoperun.event.KeyboardListener) {
	com.hoperun.event.KeyboardListener = function(appviewEditor) {
		...
		this.getKeyEditor = function(){
			if(this._keyeditor == null){
				...
				inputObj.bind(bind_name, function(){
		            ...
		            message.id = com.hoperun.util.Observer.MessageType.CURSOR_PAUSE;
		...
		this.registerListener = function(message){
			...
			$(self.appviewEditor).unbind('keydown.keyboard').unbind('keypress.keyboard').unbind('focus.keyboard').bind('keydown.keyboard', function(evt){
				...
				var isValidEvent = com.hoperun.util.BaseTool.findNearestParentNodeWithAttributeValues(obj, 'objectType', "Page,Paragraph,Line,SPAN");
	        	if(isValidEvent){
	            ...
	            if (!isValidEvent && com.hoperun.util.BaseTool.findNearestValueWithAttribute(obj, 'trackerType') == null){
	                return;
	            }
				message = new com.hoperun.util.Observer.Message();
                message.id = com.hoperun.util.Observer.MessageType.CURSOR_RECOVER;
                com.hoperun.util.Observer.sendMessage(message);
			});
		...
		this.update = function(message) {
			...
			switch(message.id){
				case com.hoperun.util.Observer.MessageType.KEYBOARD_FOCUS:
					if(this._keyEditorFocusFlag == false){
						this.resetImeStatusData();
					}
					return;
					break;
				case com.hoperun.util.Observer.MessageType.KEYBOARD_ADD:
					if(this._keyEditorFocusFlag == false){
					...
					return;
					break;
				case com.hoperun.util.Observer.MessageType.KEYBOARD_REMOVE:
					this.releaseListener();
					break;
				...
				case com.hoperun.util.Observer.MessageType.KEYBOARD_COMMAND:
					...
					switch(cmd){
						...
						case 'COPY':
							var message = new com.hoperun.util.Observer.Message();
							message.id = com.hoperun.util.Observer.MessageType.COPY_ITEM;
							...
							if (currentSelectedObj.getType() == 'Page' && !selection.isCollapsed()){
								...
								message.sender = com.hoperun.util.BaseTool.findObjWithId(currentSelectedObj.getId());
							}
						...
						case 'PASTE':
							var message = new com.hoperun.util.Observer.Message();
							message.id = com.hoperun.util.Observer.MessageType.PASTE_ITEM;
				        	com.hoperun.util.Observer.sendMessage(message);
							break;
						case 'CUT':
							var contextItem = com.hoperun.util.BaseTool.findObjWithId(currentSelectedObj.getId());
							var message = new com.hoperun.util.Observer.Message();
							message.id = com.hoperun.util.Observer.MessageType.CUT_ITEM;
							message.sender = contextItem;
				        	com.hoperun.util.Observer.sendMessage(message);
							break;
						case 'UNDO':
							if(document.getElementById("Menu_Drawing_Undo").getAttribute("canUndo")=="true")
								sendMessage(com.hoperun.util.Observer.MessageType.UNDO, null, {});
							break;
						case 'REDO':
							if(document.getElementById("Menu_Drawing_Redo").getAttribute("canRedo")=="true")
								sendMessage(com.hoperun.util.Observer.MessageType.REDO, null, {});
							break;
					}
	...
	com.hoperun.event.KeyboardListener.prototype = {
		...
		doDeleteAction: function(container, selection, flag){
			...
			if(selection && !selection.isCollapsed()){
				var textCollapsedBackspaceDeleteMessage = new com.hoperun.util.Observer.Message();
				textCollapsedBackspaceDeleteMessage.id = com.hoperun.util.Observer.MessageType.TEXT_ACTION_COLLAPSED_BACKSPACE_DELETE;
				textCollapsedBackspaceDeleteMessage.data = {'nextParaObj': nextParaObj, 'onlyDelete':flag };
				com.hoperun.util.Observer.sendMessage(textCollapsedBackspaceDeleteMessage);
			}
			else{
				if(selection.getFrom()._text.length == selection.getFromOffset()){
		    		var nextParaObj = selection.getFrom().getNextParagraph();
		    		if(nextParaObj){
		    			var paragraphDeleteMessage = new com.hoperun.util.Observer.Message();
		    			paragraphDeleteMessage.id = com.hoperun.util.Observer.MessageType.PARAGRAPH_ACTION_DELETE;
		    			paragraphDeleteMessage.data = {'nextParaObj': nextParaObj, 'onlyDelete':flag };
						com.hoperun.util.Observer.sendMessage(paragraphDeleteMessage);
		    		}
		    	}
		    	else{
					var textDeleteMessage = new com.hoperun.util.Observer.Message();
					textDeleteMessage.id = com.hoperun.util.Observer.MessageType.TEXT_ACTION_BACKSPACE_DELETE;
					textDeleteMessage.data = {'textActionFlag': '2', 'onlyDelete':flag };
					com.hoperun.util.Observer.sendMessage(textDeleteMessage);
		    	}
			}
		},
		doBackspaceAction: function(container, selection){
			if(selection && !selection.isCollapsed()){
				...
				if(selection.getFromOffset() == 0){
					...
					if(nextParaObj){
						var paragraphBackspaceMessage = new com.hoperun.util.Observer.Message();
						paragraphBackspaceMessage.id = com.hoperun.util.Observer.MessageType.PARAGRAPH_ACTION_BACKSPACE;
						paragraphBackspaceMessage.data = {'nextParaObj': nextParaObj};
						com.hoperun.util.Observer.sendMessage(paragraphBackspaceMessage);
		    		}
		    	}
		    	else{
					var textBackSpaceMessage = new com.hoperun.util.Observer.Message();
					textBackSpaceMessage.id = com.hoperun.util.Observer.MessageType.TEXT_ACTION_BACKSPACE_DELETE;
					textBackSpaceMessage.data = {'textActionFlag': '1'};
					com.hoperun.util.Observer.sendMessage(textBackSpaceMessage);
		    	}
			}
		},
		doEnterAction: function(container, selection){
			var textEnterMessage = new com.hoperun.util.Observer.Message();
			textEnterMessage.id = com.hoperun.util.Observer.MessageType.TEXT_ACTION_ENTER;
			textEnterMessage.data = {'textActionFlag': '5'};
			com.hoperun.util.Observer.sendMessage(textEnterMessage);
		},
		...
		doUpAction: function(container, selection){
			...
			var posSpan = com.hoperun.util.BaseTool.getAbsPostionInContainer(spanObj);
	    	...
	    	var nextLineDivObj = com.hoperun.util.BaseTool.getLineDivWithDirection(spanObj.parentNode, false);
	    	if(nextLineDivObj){
	    		selection = com.hoperun.model.TextSelectionModelHelper.parseWithLineAndWidth(nextLineDivObj, width);
		...
		doDownAction: function(container, selection){
	    	...
	    	var nextLineDivObj = com.hoperun.util.BaseTool.getLineDivWithDirection(spanObj.parentNode, true);
	    	if(nextLineDivObj){
	    		selection = com.hoperun.model.TextSelectionModelHelper.parseWithLineAndWidth(nextLineDivObj, width);
		    	container.setActiveSelection(selection);
	    	}
		...
		doInputAction: function(container, selection, val, isImeInputFlag){
			...
			if(this._textInputData.length == 0){
		    	...
		    	var inputTextMsg = new com.hoperun.util.Observer.Message();
		    	inputTextMsg.id = com.hoperun.util.Observer.MessageType.TEXT_ACTION_INPUT;
		    	inputTextMsg.data = data;
				com.hoperun.util.Observer.sendMessage(inputTextMsg);
	    	}
	    	else{
	    		if(isImeInputFlag){
		...
		doRightSelectAction: function(container, selection){
	    	...
	    	var hideMessage = new com.hoperun.util.Observer.Message();
			hideMessage.id = com.hoperun.util.Observer.MessageType.CURSOR_HIDE;
			com.hoperun.util.Observer.sendMessage(hideMessage);
		},
		...
		doUpSelectAction: function(container, selection){
	    	...
	    	var nextLineDivObj = com.hoperun.util.BaseTool.getLineDivWithDirection(spanObj.parentNode, false);
	    	if(nextLineDivObj){
	    		var nextSelection = com.hoperun.model.TextSelectionModelHelper.parseWithLineAndWidth(nextLineDivObj, width);
		...
		doReplaceAction:  function(container, selection, val, isImeInputFlag){
	    	...
	    	var inputTextMsg = new com.hoperun.util.Observer.Message();
		    inputTextMsg.id = com.hoperun.util.Observer.MessageType.TEXT_ACTION_REPLACE;
		    inputTextMsg.data = data;
			com.hoperun.util.Observer.sendMessage(inputTextMsg);
		}
	};
	...
	(function(){
		var keyboardListener = new com.hoperun.event.KeyboardListener();
		com.hoperun.util.Observer.register(com.hoperun.util.Observer.MessageType.KEYBOARD_FOCUS, keyboardListener);
		com.hoperun.util.Observer.register(com.hoperun.util.Observer.MessageType.KEYBOARD_ADD, keyboardListener);
		com.hoperun.util.Observer.register(com.hoperun.util.Observer.MessageType.KEYBOARD_REMOVE, keyboardListener);
