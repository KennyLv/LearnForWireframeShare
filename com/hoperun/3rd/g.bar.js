...
/**
 * This is vertical bar initialize method. 
 * 
 *  @param opts the optional parameters, the following keys are possibly set:
 *      [type     = round | sharp | soft],          default is 'square',      which is used for the out-border style of the bar.
 *      [gutter   = 'percent format string'],       default is '20%',         which is used for the bar width.
 *      [vgutter  = 'number format string'],        default is '20',          which is used for left bottom height of the bar.
 *      [stacked  = true | false],                  default is 'false',       which is used for the stack or no-stack showing the bar.
 *      [to       = 'the total size for this bar'], default is to be calculated by all data.
 *      [stretch  = true | false],                  default is not set and it should be same with 'false'.
 *      [colors   = Array],                         default is generated by Raphael
 */
Raphael.fn.g.barchart = function (x, y, width, height, values, opts) {
    ...
    chart.label = function (labels, isBottom, attrs) {
        labels = labels || [];
        attrs = attrs || {};
        ...
        if (opts.stacked) {
            for (var i = 0; i < len; i++) {
                ...
                for (var j = 0; j < (multi || 1); j++) {
                    ...
                    if (j == multi - 1) {
                        ...
                        L = paper.g.text((multi ? bars[j][i].x : bars[i].x), y + height - barvgutter / 2, label, attrs).insertBefore(covers[i * (multi || 1) + j]);
                        var bb = L.getBBox();
                        if (bb.x - 7 < l) {
                            L.remove();
                        } else {
            ...
            for (var i = 0; i < len; i++) {
                for (var j = 0; j < (multi || 1); j++) {
                    ...
                    L = paper.g.text((multi ? bars[j][i].x : bars[i].x), isBottom ? y + height - barvgutter / 2 : (multi ? bars[j][i].y : bars[i].y) - 10, label, attrs).insertBefore(covers[i * (multi || 1) + j]);
                    var bb = L.getBBox();
                    if (bb.x - 7 < l) {
                        L.remove();
                    } else {
        ...
        for (var i = 0; i < len; i++) {
            for (var j = 0; j < multi; j++) {
                ...
                var X = isRight ? bars[i * (multi || 1) + j].x - barheight / 2 + 3 : x + 5,
                    A = isRight ? "end" : "start",
                    L;
                this.labels.push(L = paper.g.text(X, bars[i * (multi || 1) + j].y, label).attr({"text-anchor": A, "writing-mode": "rl"}).insertBefore(covers[0]));
                if (L.getBBox().x < x + 5) {
                    L.attr({x: x + 5, "text-anchor": "start", "writing-mode": "rl"});
                } else {
