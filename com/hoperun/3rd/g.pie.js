...
/**
 * Draw pie chart.
 *  The default colors come from Raphael.g.colors, which could be pre-set with [Raphael.fn.g.colors]
 *  
 * @param cx     the x of the center 
 * @param cy     the y of the center
 * @param r      the radius
 * @param values the array of data
 * @param opts   the optional 
 *      stroke,      the stroke color
 *      strokewidth, the stroke width
 *      colors,      the color of each part, when it is not prepared default color would be picked up
 *      partsOffset, the offset of each part
 *      
 */
Raphael.fn.g.piechart = function (cx, cy, r, values, opts) {
    ...
    if(opts.partsOffset){
        for(var i=0; i < values.length; i++){
            var val = 0;
            for(var j=0; j<opts.partsOffset.length; j++){
                if(opts.partsOffset[j].index == i){
                    val = opts.partsOffset[j].value;
                    break;
                }
            }
            partsOffset.push(val);
        }
    }
    if (len == 1) {
        ...
        function sector(cx, cy, r, startAngle, endAngle, fill, offset) {
            var rad = Math.PI / 180;
            if(offset){
                cx += offset * Math.cos((-startAngle * rad + -endAngle * rad)/2);
                cy += offset * Math.sin((-startAngle * rad + -endAngle * rad)/2);
            }
        ...
        for (i = 0; i < len; i++) {
            ...
            var endAngle = angle - 360 * values[i] / total;
            if (opts.init) {
                var ipath = sector(cx, cy, 1, angle, angle - 360 * values[i] / total).join(",");
            }
            var path = sector(cx, cy, r, angle, angle -= 360 * values[i] / total, null, partsOffset[i]);
