 ...
 * @import com.hoperun.util.BaseTool,com.hoperun.util.NodeCache,com.hoperun.util.TextStyleContainerUtils, com.hoperun.model.SvgStyle
 * @author lu_feng
 */
com.hoperun.render.SvgTextRender = function(container){
...
com.hoperun.render.SvgTextRender.prototype = {
	_id: null,
	_container: null,
	...
	_getDefaultStyle: function(){
		...
		if(stylesData && stylesData.styles && stylesData.styles.length>0){
			...
			style = new com.hoperun.model.SvgStyle();
		}
		return style;
	},
				...
				for(var j=0; j<paraData.length; j++){
					...
					for(var k=0; k<lineData.data.length; k++){
						...
						if(idx + text.length >= offset){
							com.hoperun.util.SvgTextHelper.changeStyle(style);
							return com.hoperun.util.SvgTextHelper.getWidth(text.substring(0, offset - idx));
						}
						idx += text.length;
					}
				}
	...
	render: function(svgText, totalWidth, zoom, strokeWidth){
		...
		for(var i=0; i<textArr.length; i++){
			renderData.push(com.hoperun.util.SvgTextHelper.getLinesOfParagraph(textArr[i], stylesData, this._getDefaultStyle(), totalWidth-strokeWidth*2, offset));
		...
		if(height > this._container.getHeight()){
			...
			var layoutListener = new com.hoperun.util.Observer.Message();
			layoutListener.id = com.hoperun.util.Observer.MessageType.SHAPE_LAYOUT;
			layoutListener.sender = this._container;
			layoutListener.data = {};
			com.hoperun.util.Observer.sendMessage(layoutListener);
		}
